...
def lambda_handler(event, context):
        ...
        if Lifecycle_Transition == "autoscaling:EC2_INSTANCE_LAUNCH":
                ...
                if update_iam_managed_policy := update_managed_policy(
                    MANAGED_POLICY_ARN, instance_id):
                        print("SUCCESS")
                else:
...
def update_iam_inline_policy(instance_id):
        try:
                ...
                log(f"Error updating the inline policy {instance_id}: {e.response['Error']}") 
...
def update_secret_manager_policy(instance_id):
        try:
                ...
                log(f"Error updating the secret manager policy {instance_id}: {e.response['Error']}"
                    )	
...
def update_s3_bucket_policy(bucket_name,instance_id):
        try:
                ...
                log(f"Error updating S3 bucket policy {instance_id}: {e.response['Error']}")	
...
def update_managed_policy(policy_arn,instance_id):
        ...
        try:
                ...
                log(f"Error updating IAM managed policy {instance_id}: {e.response['Error']}")
        return var4	
